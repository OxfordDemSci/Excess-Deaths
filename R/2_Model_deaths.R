...
# add number of flu years since origin (starts at 0)
Data.dt[,
        flu.years.since.origin :=
          cumsum(week == cnst$fluyear.week.start),
        by = list(sex, age.n)
        ]
...
Data.dt[,`:=`(
  ...
  flu.years.since.origin.fct = as.factor(flu.years.since.origin)
)]
...
Deaths.UK.for.plot[,`:=`(
  ...
  age.n =
    factor(
      age.n,
      c(0, 15, 45, 65, 75, 85),
      c('[0,15)', '[15,45)', '[45,65)', '[65,75)', '[75,85)', '85+')
    )
)]
