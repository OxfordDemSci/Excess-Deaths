...
update.deaths <- data.table(melt(update.deaths, id.vars = c('age','agegroup','sex'),
                                 variable.name = 'week',value.name = 'deaths',
                                 variable.factor = 'F'))
...
# weeks since origin
Data.dt[,time := WeeksSinceOrigin(date, glob$origin_date)]

# date epi year starts
Data.dt[, start.of.epi.year :=
          ISOWeekDate2Date(
            ifelse(iso.week<=glob$week_epi_year_starts, year-1, year),
...
Data.dt <- merge(Data.dt, weeks.inter)
Data.dt[, `:=`(
  week21 = week == 21,
  last_week = (week + 1) >= 52 & (week + 1) == max.week,
  first_week = week == 0
)]
